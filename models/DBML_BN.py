...
class PSM_BN(nn.Module):
    def __init__(self, user_size, item_size, word_size, embedding_dim,\
        ...
        self.userTimeMeanBN = nn.BatchNorm1d(self.embedding_dim)
        self.userTimeStdBN = nn.BatchNorm1d(self.embedding_dim)
        self.queryMeanBN = nn.BatchNorm1d(self.max_query_len)
        self.wordTimeMeanBN = nn.BatchNorm1d(self.max_review_len)
        self.wordTimeStdBN = nn.BatchNorm1d(self.max_review_len)
        self.wordNegTimeMeanBN = nn.BatchNorm1d(self.neg_num)
        self.wordNegTimeStdBN = nn.BatchNorm1d(self.neg_num)
        self.itemTimeMeanBN = nn.BatchNorm1d(self.embedding_dim)
        self.itemTimeStdBN = nn.BatchNorm1d(self.embedding_dim)
        self.itemNegTimeMeanBN = nn.BatchNorm1d(self.neg_num)
        self.itemNegTimeStdBN = nn.BatchNorm1d(self.neg_num)
        ...
        self.time2std = nn.Linear(self.embedding_dim*2, self.embedding_dim)
...
#         self.itemDecoder = nn.Linear(self.embedding_dim, self.embedding_dim)
    ...
    def forward(self, user, item_pos, query, query_len, word, word_len, times, items_neg, word_neg):
        ...
        user_mean_pri = self.userTimeMeanBN(user_mean_pri)
        user_mean = self.time2mean(torch.cat([user_mean, time_laten], 1))
        user_mean = self.userTimeMeanBN(user_mean)
        
        user_std = self.user_std(user).squeeze(1) # (batch, out_size)
        user_std_pri = self.time2std(torch.cat([user_std, pri_time_laten], 1)).mul(0.5).exp()
        user_std_pri = self.userTimeStdBN(user_std_pri)
        user_std = self.time2std(torch.cat([user_std, time_laten], 1)).mul(0.5).exp()
        user_std = self.userTimeStdBN(user_std)
        ...
        word_mean_pos_pri = self.wordTimeMeanBN(word_mean_pos_pri)
        word_mean_pos = self.time2mean(torch.cat([word_mean_pos, time_laten.unsqueeze(1).expand_as(word_mean_pos)], 2))
        word_mean_pos = self.wordTimeMeanBN(word_mean_pos)
        
        word_std_pos = self.wordEmbedding_std(word)
        word_std_pos_pri = self.time2std(torch.cat([word_std_pos, pri_time_laten.unsqueeze(1).expand_as(word_std_pos)], 2)).mul(0.5).exp()
        word_std_pos_pri = self.wordTimeStdBN(word_std_pos_pri)
        word_std_pos = self.time2std(torch.cat([word_std_pos, time_laten.unsqueeze(1).expand_as(word_std_pos)], 2)).mul(0.5).exp()
        word_std_pos = self.wordTimeStdBN(word_std_pos)
        ...
        word_mean_neg_pri = self.wordNegTimeMeanBN(word_mean_neg_pri)
        word_mean_neg = self.time2mean(torch.cat([word_mean_neg, time_laten.unsqueeze(1).expand_as(word_mean_neg)], 2))
        word_mean_neg = self.wordNegTimeMeanBN(word_mean_neg)
        
        word_std_neg = self.wordEmbedding_std(word_neg)
        word_std_neg_pri = self.time2std(torch.cat([word_std_neg, pri_time_laten.unsqueeze(1).expand_as(word_std_neg)], 2)).mul(0.5).exp()
        word_std_neg_pri = self.wordNegTimeStdBN(word_std_neg_pri)
        word_std_neg = self.time2std(torch.cat([word_std_neg, time_laten.unsqueeze(1).expand_as(word_std_neg)], 2)).mul(0.5).exp()    
        word_std_neg = self.wordNegTimeStdBN(word_std_neg)
        ...
        item_mean_pos_pri =self.itemTimeMeanBN(item_mean_pos_pri)
        item_mean_pos = self.time2mean(torch.cat([item_mean_pos, time_laten], 1))
        item_mean_pos = self.itemTimeMeanBN(item_mean_pos)
        
        item_std_pos = self.item_std(item_pos).squeeze(1) # (batch, out_size)
        item_std_pos_pri = self.time2std(torch.cat([item_std_pos, pri_time_laten], 1)).mul(0.5).exp()
        item_std_pos_pri = self.itemTimeStdBN(item_std_pos_pri)
        item_std_pos = self.time2std(torch.cat([item_std_pos, time_laten], 1)).mul(0.5).exp()
        item_std_pos = self.itemTimeStdBN(item_std_pos)
        ...
        items_mean_neg_pri = self.itemNegTimeMeanBN(items_mean_neg_pri)
        items_mean_neg = self.time2mean(torch.cat([items_mean_neg, time_laten.unsqueeze(1).expand_as(items_mean_neg)], 2))
        items_mean_neg = self.itemNegTimeMeanBN(items_mean_neg)
        
        items_std_neg = self.item_std(items_neg)# (batch, neg_sample_num, out_size)
        items_std_neg_pri = self.time2std(torch.cat([items_std_neg, pri_time_laten.unsqueeze(1).expand_as(items_std_neg)], 2)).mul(0.5).exp()
        items_std_neg_pri = self.itemNegTimeStdBN(items_std_neg_pri)
        items_std_neg = self.time2std(torch.cat([items_std_neg, time_laten.unsqueeze(1).expand_as(items_std_neg)], 2)).mul(0.5).exp()
        items_std_neg = self.itemNegTimeStdBN(items_std_neg)
